...
##    print(sortlist)
    for idx in range(len(xyzlist)):
        for jdx in range(idx+1, len(xyzlist)):
            if xyzlist[sortlist[idx]] < xyzlist[sortlist[jdx]] - 1.80: break
            retlist.add((max(sortlist[idx],sortlist[jdx]),min(sortlist[idx],sortlist[jdx])))
    return retlist

def get_line(mol):
    length = lambda a,b: (a[0]-b[0])**2+(a[1]-b[1])**2+(a[2]-b[2])**2
    line = lambda a,b: (np.linspace(a[0],b[0],100),np.linspace(a[1],b[1],100),np.linspace(a[2],b[2],100))
    
    xyzlist = np.array([i.xyz for i in mol])
    idxlist = judge_line(xyzlist[:,0]) | judge_line(xyzlist[:,1]) | judge_line(xyzlist[:,2])
##    print(idxlist)
    retlist = [line(xyzlist[idx[0]],xyzlist[idx[1]]) for idx in idxlist if length(xyzlist[idx[0]],xyzlist[idx[1]])<3.2]
...
##        points3d(float(m[1]), float(m[2]), float(m[3]), resolution=12)
        mlab.points3d(*m.xyz, color=m.color, scale_factor=m.size, resolution=12,opacity=0.95)
